...
public abstract class SwitcherScreen<T extends Type<T>> extends Screen {
    ...
    public class TypeSlot extends AbstractWidget {
        ...
        @Override
        public void renderButton(PoseStack stack, int $$1, int $$2, float $$3) {
            RenderSystem.setShader(GameRenderer::getPositionTexShader);
            RenderSystem.setShaderTexture(0, LOCATION);
            stack.pushPose();
            stack.translate(this.x, this.y, 0.0D);
            blit(stack, 0, 0, 0.0F, 75.0F, 26, 26, 128, 128);
            if (this.isSelected) blit(stack, 0, 0, 26.0F, 75.0F, 26, 26, 128, 128);
            stack.popPose();
            this.type.draw(SwitcherScreen.this.itemRenderer, this.x + 5, this.y + 5);
        }
